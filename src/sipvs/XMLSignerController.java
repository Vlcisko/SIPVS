...
public class XMLSignerController implements Initializable {
	...
	public void signFiles() throws Exception {
		...
		// if(selectedXmlFile != null && selectedXsdFile != null && selectedXslFile !=
		// null) {
		if (true) {
			// testovacie data
			...
			Object xmlObject = xmlPlugin.CreateObject2("id", "XML", readFile(pathXML.getText()),
					readFile(pathXSD.getText()), "", "https://something.com/example.xsd", readFile(pathXSL.getText()),
					"https://something.com/example.xsl", "HTML");
			...
			if (resOfSign != 0) {
				...
				showAlert(
						"XML úspešne podpísané, riadenie programu bude presmerované. Zvoľte lokáciu pre uloženie súboru.");
